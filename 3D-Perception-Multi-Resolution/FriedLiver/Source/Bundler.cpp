...
void Bundler::predictMask()
{
	//这里新增一个maskrcnn，测试速度
	//std::cout << "predictMask" << std::endl;
...
bool Bundler::optimize(unsigned int numNonLinIterations, unsigned int numLinIterations, bool bUseVerify, bool bRemoveMaxResidual, bool bIsScanDone, bool& bOptRemoved)
{
	...
	if (m_optimizer.useVerification()) {
		...
		{ 
			cudaDeviceSynchronize(); m_timer.stop(); TimingLog::getFrameTiming(true).timeSolve += m_timer.getElapsedTimeMS(); 
		}
		if (valid > 0)
			ret = true;

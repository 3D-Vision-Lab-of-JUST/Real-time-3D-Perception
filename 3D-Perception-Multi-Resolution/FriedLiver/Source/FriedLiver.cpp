...
void bundlingThreadFunc() 
{
	...
	while (1) {
		...
		{
			int a = g_bundler->getCurrProcessedFrame();
		...
		}
		{
			...
			while (g_bundler->hasProcssedInputFrame()) 
			{ 
				//wait until depth sensing has confirmed the last one (WAITING THAT DEPTH SENSING RELEASES ITS LOCK)
			...
			}
			{
				...
				g_bundler->processInput(isUsePose_GT);						//perform sift and whatever
			}
...
int main(int argc, char** argv)
{
	...
	try {
		...
		std::cout << "s_integrationWidth:" << GlobalAppState::get().s_integrationWidth << ", s_integrationHeight:" << GlobalAppState::get().s_integrationHeight << std::endl;
...
//{
//	ml::SensorData sd;
//	sd.loadFromImages("D:/Bussiness/JUST/bundlefusion/dataset/copyroom/copyroom", "frame-", "jpg");
//	sd.saveToFile("D:/Bussiness/JUST/bundlefusion/dataset/copyroom/copyroom/test.sens");
//}

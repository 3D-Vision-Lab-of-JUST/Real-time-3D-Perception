...
__global__ void resetHeapKernel(HashDataStruct hashData) 
{
	...
	if (idx == 0) {
		hashData.d_heapCounter[0] = hashParams.m_numSDFBlocks - 1;	//points to the last element of the array -> 100000 - 1
	}
	
	if (idx < hashParams.m_numSDFBlocks) {
		
		hashData.d_heap[idx] = hashParams.m_numSDFBlocks - idx - 1;
		hashData.deleteBlock(idx);
...
__global__ void resetRetouchKernel(HashDataStruct hashData)
{
	const HashParams& hashParams = c_hashParams;
	const unsigned int idx = blockIdx.x;
	if (idx == 0) {
		hashData.d_retouchCounter[0] = hashParams.m_numSDFBlocks / 5 - 1;	//points to the last element of the array -> 100000 - 1
	}
	hashData.d_retouchDecision[idx] = hashParams.m_numSDFBlocks / 5 - idx - 1;
}
extern "C" void resetCUDA(HashDataStruct& hashData, const HashParams& hashParams)
{
	...
	{
		//resetting the mutex
		const dim3 gridSize(hashParams.m_numSDFBlocks / 5, 1);
		const dim3 blockSize(1, 1);

		resetRetouchKernel << <gridSize, blockSize >> >(hashData);
...
bool isSDFBlockStreamedOut(const int3& sdfBlock, const HashDataStruct& hashData, const unsigned int* d_bitMask)	//TODO MATTHIAS (-> move to HashData)
{
	...
	float3 posWorld = hashData.virtualWorldPosToWorld(hashData.SDFBlockToVirtualWorldPos(sdfBlock)); // sdfBlock is assigned to chunk by the bottom right sample pos
...
__global__ void retouchBlocksKernel(HashDataStruct hashData)
{
	...
	if (hashData.d_SDFBlocks[blockIdx].m_bIsRetouched && hashData.d_SDFBlocks[blockIdx].m_bRetouchIndex == -1)
	{
		uint oldCounter = atomicSub(&hashData.d_retouchCounter[0], 1);
		//printf("alloc retouch block:%d\n", (int)oldCounter);
		//printf("blockId: %d, retouch start\n", (int)blockIdx);
		uint currIndex = hashData.d_retouchDecision[oldCounter];
		//printf("currIndex: %d\n", (int)currIndex);
		/*for (int i = 0; i <SDF_BLOCK_SIZE * SDF_BLOCK_SIZE * SDF_BLOCK_SIZE; i++)
		{
				
			Voxel v = hashData.d_SDFBlocks[blockIdx].voxels[i];
					
			hashData.d_SDFBlocks_Retouched[currIndex].voxels[i * 8].color = v.color;
			hashData.d_SDFBlocks_Retouched[currIndex].voxels[i * 8].weight = v.weight;
			hashData.d_SDFBlocks_Retouched[currIndex].voxels[i * 8].sdf = v.sdf;

		}*/
		hashData.d_SDFBlocks[blockIdx].m_bRetouchIndex = currIndex;
		hashData.d_retouchDecision[oldCounter] = -1;
	}
}


extern "C" void retouchBlocksCUDA(HashDataStruct& hashData, const HashParams& hashParams)
{
	if (hashParams.m_numOccupiedBlocks == 0)
	{
		return;
	}
	//allocRetouch(hashData, hashParams);
	const dim3 gridSize(hashParams.m_numOccupiedBlocks, 1);
	const dim3 blockSize(1, 1);
	//allocRetouchBlock(hashData, hashParams);
	//cudaMalloc(&d_hashCompactified, sizeof(HashEntry)* params.m_hashNumBuckets * params.m_hashBucketSize);
	retouchBlocksKernel <<<gridSize, blockSize>>> (hashData);
...
__global__ void integrateDepthMapKernel(HashDataStruct hashData, DepthCameraData cameraData) {
	...
	uint blockIdx = entry.ptr;
	uint voxelIdx = threadIdx.x;	//inside of an SDF block
	float3 pi_base = hashData.SDFBlockToVirtualWorldPos(entry.pos);
	
	//这里要改为区分是否细化的
	float3 pi = pi_base + make_float3(hashData.delinearizeVoxelIndex(voxelIdx));
	...
	for (int i = 0; i < retouchedNum; i++)
	{
		...
		float3 pf = hashData.virtualWorldPosToWorld(newpi);

		pf = hashParams.m_rigidTransformInverse * pf;////////////////////////////////////////////???????????
		uint2 screenPos = make_uint2(cameraData.cameraToKinectScreenInt(pf));
		/////////////////////////////////////////////
		const uint* maskData;
		if (cameraData.d_hasMaskData)
		{
			maskData = cameraData.d_maskData;
		}

		if (screenPos.x < cameraParams.m_imageWidth && screenPos.y < cameraParams.m_imageHeight)
		{	//on screen

			uint num = screenPos.y * cameraParams.m_imageWidth * 2 * 2 + screenPos.x * 2;//这里是计算对应编号
			uint maskIdx = 0;
			if (cameraData.d_hasMaskData)
			{
				//printf("num:%d, x:%d, y:%d\n", num, screenPos.x, screenPos.y);
				maskIdx = maskData[num];
			}
			if (maskIdx > 0)
			{
				//细化!!!!!
				//首先找到需要细化的block编号，
				if (!hashData.d_SDFBlocks[blockIdx].m_bIsRetouched)
				{
					hashData.d_SDFBlocks[blockIdx].m_bIsRetouched = true;
				}
			}
			//uint maskIdx = 0;
			...
			if (color.x != MINF && depth != MINF) { // valid depth and color
				...
				if (depth < hashParams.m_maxIntegrationDistance) {
					...
					if (abs(sdf) < truncation)
					{
						...
						{
							curr.color = make_uchar4(0, 255, 0, 0);//////////curr.color = make_uchar4(0,255,0,0);
						}
						Voxel oldVoxel;
						if (hashData.d_SDFBlocks[blockIdx].m_bRetouchIndex!=-1)
						{
							oldVoxel = hashData.d_SDFBlocks_Retouched[hashData.d_SDFBlocks[blockIdx].m_bRetouchIndex].voxels[voxelIdx + i];

							//oldVoxel = hashData.d_SDFBlocks[blockIdx].voxels_retouched[voxelIdx + i];
						}
						else {
							oldVoxel = hashData.d_SDFBlocks[blockIdx].voxels[voxelIdx];
						}

						Voxel newVoxel;
						///////////////////////////////////////////////!!!!
						uint oldMask = oldVoxel.color.w;
						...
						{	//integration
							...
							//newVoxel.color = make_uchar4(res.x, res.y, res.z, maskIdx);
							...
							if (newVoxel.sdf >= 0.02 || newVoxel.sdf <= -0.02)
							{
								maskIdx = 0;
							}
							if (maskIdx > 0 && oldMask == 0)
							{
								//若有值，则更新
								newVoxel.color = make_uchar4(res.x, res.y, res.z, maskIdx);//0,0,0
							}
							else
							{
								//若无值，则用旧值。若旧值没有，则写0
								if (oldMask > 0)
								{
									newVoxel.color = make_uchar4(res.x, res.y, res.z, oldMask);//0,0,0
								}
								else {
									newVoxel.color = make_uchar4(res.x, res.y, res.z, oldMask);
								}

							}
							//newVoxel.maskIdx = maskIdx;
						}
						else
						{	//deintegration
							//float3 res = 2 * c0 - c1;
							...
							if (newVoxel.weight <= 0.001f) {
								newVoxel.sdf = 0.0f;
								newVoxel.color = make_uchar4(0, 0, 0, oldMask);
						...
						if (hashData.d_SDFBlocks[blockIdx].m_bRetouchIndex!=-1)
						{
							hashData.d_SDFBlocks_Retouched[hashData.d_SDFBlocks[blockIdx].m_bRetouchIndex].voxels[voxelIdx + i] = newVoxel;

							if (i == 0)
							{
								hashData.d_SDFBlocks[blockIdx].voxels[voxelIdx/8] = newVoxel;
							}
							//hashData.d_SDFBlocks[blockIdx].voxels_retouched[voxelIdx + i] = newVoxel;
						}
						else {
							hashData.d_SDFBlocks[blockIdx].voxels[voxelIdx] = newVoxel;
						}
						//printf("blockIdx:%d, voxelIdx:%d, 22\n", blockIdx, voxelIdx);
					}
				}
			}
		}
...
//	int weight = hashData.d_SDFBlocks[entry.ptr].voxels[threadIdx.x].weight;
//	weight = max(0, weight-1);	
//	hashData.d_SDFBlocks[entry.ptr].voxels[threadIdx.x].weight = weight;
//}
//
...
__global__ void garbageCollectIdentifyKernel(HashDataStruct hashData) {
	...
	Voxel v0 = hashData.d_SDFBlocks[entry.ptr].voxels[2 * threadIdx.x + 0];
	Voxel v1 = hashData.d_SDFBlocks[entry.ptr].voxels[2 * threadIdx.x + 1];
...
__global__ void garbageCollectFreeKernel(HashDataStruct hashData) {
	...
	if (hashIdx < c_hashParams.m_numOccupiedBlocks && hashData.d_hashDecision[hashIdx] != 0) {	//decision to delete the hash entry
		...
		//printf("garbage collect: %d", (int)hashIdx);
		const HashEntry& entry = hashData.d_hashCompactified[hashIdx];
		//if (entry.ptr == FREE_ENTRY) return; //should never happen since we did compactify before

		if (hashData.deleteHashEntryElement(entry.pos)) {	//delete hash entry from hash (and performs heap append)
			
			//uint linBlockSize = SDF_BLOCK_SIZE * SDF_BLOCK_SIZE * SDF_BLOCK_SIZE;

			/*if (hashData.isBlockRetouched(hashData.SDFBlockToWorld(entry.pos)))
			{
				linBlockSize = linBlockSize * 8;
			}*/
			#pragma unroll 1

			hashData.deleteBlock(entry.ptr);

...
__global__ void rayIntervalSplatKernel(HashDataStruct hashData, RayCastData rayCastData) 
{
	...
	if (entry.ptr != FREE_ENTRY) {
		...
		float voxelSize;
		int block_size;
		bool isRetouched = hashData.isBlockRetouched(hashData.worldToVirtualWorldPosFloat(worldCurrentVoxel));
		if (isRetouched)
		{
			voxelSize = c_hashParams.m_virtualVoxelSize / 2;
			block_size = SDF_BLOCK_SIZE *2;
		}
		else {
			voxelSize = c_hashParams.m_virtualVoxelSize;
			block_size = SDF_BLOCK_SIZE;
		}
		float3 MINV = worldCurrentVoxel - voxelSize / 2.0f;

		float3 maxv = MINV+ block_size*voxelSize;

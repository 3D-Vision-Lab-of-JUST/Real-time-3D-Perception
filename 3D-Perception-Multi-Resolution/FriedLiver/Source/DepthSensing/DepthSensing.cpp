...
CDXUTTextHelper*            g_pLocationHelper = NULL;
...
{
	startMaskSensing(sensor, imageManager);
...
void RenderHelp()
{
	...
	g_pTxtHelper->DrawTextLine(L"Locations ");

	for (int i = 0; i < m_xs.size(); i++)
	{

		g_pTxtHelper->DrawTextLine("instance"+std::to_string(i)+": x:"+std::to_string(m_xs[i])+", y:" + std::to_string(m_ys[i]) + ", z:" + std::to_string(m_zs[i]));
	}
...
HRESULT CALLBACK OnD3D11CreateDevice(ID3D11Device* pd3dDevice, const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext)
{
	...
	g_pLocationHelper = new CDXUTTextHelper(pd3dDevice, pd3dImmediateContext, &g_DialogResourceManager, 15);
...
void visualizeFrame(ID3D11DeviceContext* pd3dImmediateContext, ID3D11Device* pd3dDevice, const mat4f& transform, bool trackingLost)
{
	...
	if (GlobalAppState::get().s_RenderMode == 1)	
	{
		...
		vector<float> ys;
		vector<float> zs;
		g_RGBDRenderer.RenderDepthMap(pd3dImmediateContext, g_rayCast->getRayCastData().d_depth, g_rayCast->getRayCastData().d_colors, g_rayCast->getRayCastParams().m_width, g_rayCast->getRayCastParams().m_height, g_rayCast->getIntrinsicsInv(), view, renderIntrinsics, g_CustomRenderTarget.getWidth(), g_CustomRenderTarget.getHeight(), GlobalAppState::get().s_renderingDepthDiscontinuityThresOffset, GlobalAppState::get().s_renderingDepthDiscontinuityThresLin, xs, ys, zs);
		
		m_xs = xs;
		m_ys = ys;
		m_zs = zs;
		/*g_pLocationHelper->Begin();
		g_pLocationHelper->SetInsertionPos(2, 40);
		g_pLocationHelper->SetForegroundColor(D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f));
		g_pLocationHelper->DrawTextLine(L"Locations ");
		for (int i = 0; i < xs.size(); i++)
		{
			g_pLocationHelper->DrawTextLine(L"  \tx:\t y:\t z:");
		}
		g_pLocationHelper->End();*/
		//渲染mask的boundingbox
		//g_RGBDRenderer.RenderMaskBox(pd3dImmediateContext, g_rayCast->getRayCastData().d_depth, g_rayCast->getRayCastData().d_colors, g_rayCast->getRayCastParams().m_width, g_rayCast->getRayCastParams().m_height, g_rayCast->getIntrinsicsInv(), view, renderIntrinsics, g_CustomRenderTarget.getWidth(), g_CustomRenderTarget.getHeight(), GlobalAppState::get().s_renderingDepthDiscontinuityThresOffset, GlobalAppState::get().s_renderingDepthDiscontinuityThresLin);
...
void reintegrate()
{
	...
	for (unsigned int fixes = 0; fixes < maxPerFrameFixes; fixes++) 
	{
		...
		if (tm->getTopFromDeIntegrateList(oldTransform, frameIdx)) {
			...
			if (g_CudaImageManager->getIntegrateFrame(frameIdx).hasMaskRdy())
			{
				depthCameraData = DepthCameraData(g_CudaImageManager->getIntegrateFrame(frameIdx).getDepthFrameGPU(), g_CudaImageManager->getIntegrateFrame(frameIdx).getColorFrameGPU(), g_CudaImageManager->getIntegrateFrame(frameIdx).getMaskFrame());
			}
			else {
			...
			//DepthCameraData depthCameraData(f.getDepthFrameGPU(), f.getColorFrameGPU(), f.getMaskFrame());
...
void CALLBACK OnD3D11FrameRender(ID3D11Device* pd3dDevice, ID3D11DeviceContext* pd3dImmediateContext, double fTime, float fElapsedTime, void* pUserContext)
{
	...
	vec4uc* colorInput = g_depthSensingRGBDSensor->getColorRGBX();
	
	cv::Mat colorMat = ucharToMat(colorInput);
	MaskSensing(colorMat, g_CudaImageManager->getCurrFrameNumber());
	if (g_CudaImageManager->getCurrFrameNumber() % 10 == 0)
	{
		if (g_CudaImageManager->getCurrFrameNumber() < 10)
		{
			vec4uc* colorInput = g_depthSensingRGBDSensor->getColorRGBX();
			//m_RGBDSensor->getColorRGBX();
			// = m_ImageManager->GetColorInput();
			cv::Mat colorMat = ucharToMat(colorInput);
			MaskSensing(colorMat, g_CudaImageManager->getCurrFrameNumber());
		}
		else 
		{
			...
			// = m_ImageManager->GetColorInput();
			cv::Mat colorMat = ucharToMat(colorInput);
			std::thread t(MaskSensing, colorMat, g_CudaImageManager->getCurrFrameNumber());
			//这里有个问题，如何将某一帧的mask和这一帧的深度图绑定起来，会不会出现已经使用深度图了 但mask还没来得及算的情况？
...
void renderToFile(ID3D11DeviceContext* pd3dImmediateContext, const mat4f& lastRigidTransform, bool trackingLost)
{
	...
	{	// reconstruction
		...
		g_RGBDRenderer.RenderDepthMap(pd3dImmediateContext, g_rayCast->getRayCastData().d_depth, g_rayCast->getRayCastData().d_colors,
			...
			GlobalAppState::get().s_renderingDepthDiscontinuityThresOffset, GlobalAppState::get().s_renderingDepthDiscontinuityThresLin, xs, ys, zs);
		g_CustomRenderTarget.Unbind(pd3dImmediateContext);

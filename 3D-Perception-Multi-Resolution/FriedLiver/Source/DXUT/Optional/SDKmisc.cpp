...
HRESULT CDXUTTextHelper::DrawTextLine(std::string strMsg)
{
	if (NULL == m_pFont9 && NULL == m_pd3d11DeviceContext)
		return DXUT_ERR_MSGBOX(L"DrawTextLine", E_INVALIDARG);

	size_t origsize = strMsg.length() + 1;
	const size_t newsize = 100;
	size_t convertedChars = 0;
	wchar_t *wcstring = (wchar_t *)malloc(sizeof(wchar_t)*(strMsg.length() - 1));
	mbstowcs_s(&convertedChars, wcstring, origsize, strMsg.c_str(), _TRUNCATE);
	...
	SetRect(&rc, m_pt.x, m_pt.y, 0, 0);
	if (m_pFont9)
		hr = m_pFont9->DrawText(m_pSprite9, wcstring, -1, &rc, DT_NOCLIP, m_clr);
	else if (m_pd3d11DeviceContext)
		DrawText11DXUT(m_pd3d11Device, m_pd3d11DeviceContext, wcstring, rc, m_clr,
		(float)m_pManager->m_nBackBufferWidth, (float)m_pManager->m_nBackBufferHeight, false);

...
{
	m_cudaImageManager->copyToBundling(m_input.d_inputDepthRaw, m_input.d_inputDepthFilt, m_input.d_inputColor, m_input.d_poseGT);
...
void OnlineBundler::computeCurrentSiftTransform(bool bIsValid, unsigned int frameIdx, unsigned int localFrameIdx, unsigned int lastValidCompleteTransform, bool isUsePose_GT)
{
	if (isUsePose_GT)
	{
		m_currIntegrateTransform[frameIdx].setZero(-std::numeric_limits<float>::infinity());
		MLIB_CUDA_SAFE_CALL(cudaMemcpy(&m_currIntegrateTransform[frameIdx], m_input.d_poseGT, sizeof(mat4f), cudaMemcpyDeviceToHost));
...
void OnlineBundler::processInput(bool isUsePose_GT)
{
	const unsigned int curFrame = m_cudaImageManager->getCurrFrameNumber();
	const bool bIsLastLocal = isLastLocalFrame(curFrame);
	if (curFrame > 0 && m_state.m_lastFrameProcessed == curFrame) 
	{ //sequence has ended (no new frames from cudaimagemanager)
	...
	{ 
		cudaDeviceSynchronize(); m_timer.start(); 
	}
	...
	if (curLocalFrame > 0) {
		...
		computeCurrentSiftTransform(m_state.m_bLastFrameValid, curFrame, curLocalFrame, m_state.m_lastValidCompleteTransform, isUsePose_GT);
...
void OnlineBundler::processGlobal()
{
	...
	if (processState == BundlerState::PROCESS) 
	{
		//if (m_global->getNumFrames() <= m_state.m_lastLocalSolved) {
			...
			if (GlobalBundlingState::get().s_enableGlobalTimings) 
			{ 
				cudaDeviceSynchronize(); m_timer.stop(); 

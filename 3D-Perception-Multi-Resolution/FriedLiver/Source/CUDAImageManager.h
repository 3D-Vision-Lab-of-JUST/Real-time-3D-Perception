...
public:
	...
	public:
		...
		const uint* getMaskFrame()
		{
			return m_maskData;
		}
		const bool hasMaskRdy()
		{
			return m_isHasMaskData;
		}
		...
		void SetMaskData(uint* maskData)
		{
			m_maskData = maskData;
			m_isHasMaskData = true;
		}
	...
	private:
		...
		uint*   m_maskData;//存放maskrcnn的结果 640*480
		bool    m_isHasMaskData = false;
	...
	CUDAImageManager(unsigned int widthIntegration, unsigned int heightIntegration, unsigned int widthSIFT, unsigned int heightSIFT, RGBDSensor* sensor, bool storeFramesOnGPU = false) 
	{
		...
		MLIB_CUDA_SAFE_CALL(cudaMalloc(&d_poseInput, sizeof(mat4f)));
	...
	~CUDAImageManager() {
		...
		MLIB_CUDA_SAFE_FREE(d_poseInput);
	...
	uchar4* GetColorInput()
	{
		return d_colorInput;
	}

	void copyToBundling(float* d_depthRaw, float* d_depthFilt, uchar4* d_color, mat4f* d_pose) const {
		...
		MLIB_CUDA_SAFE_CALL(cudaMemcpy(d_pose, d_poseInput, sizeof(mat4f), cudaMemcpyDeviceToDevice));
	}
	...
	void SetMaskData(uint* maskData, int currentFrameIdx)
	{
		uint* data;
		MLIB_CUDA_SAFE_CALL(cudaMalloc(&data, sizeof(uint) * 640 * 480));
		MLIB_CUDA_SAFE_CALL(cudaMemcpy(data, maskData, sizeof(uint) * 640 * 480, cudaMemcpyHostToDevice));
		
		m_data[currentFrameIdx].SetMaskData(data);
	}
private:
